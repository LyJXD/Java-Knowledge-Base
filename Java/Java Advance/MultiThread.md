...
- **新建状态（New）**
	**New**表示线程被创建但尚未启动的状态：当我们用 `new Thread()` 新建一个线程时，如果线程没有开始运行 `start()` 方法，那么线程也就没有开始执行 `run()` 方法里面的代码，那么此时它的状态就是**New**。而一旦线程调用了 `start()`，它的状态就会从**New**变成**Runnable**。
- **可运行状态（Runnable）**
	Java中的**Runable**状态对应操作系统线程状态中的两种状态，分别是**Running**和**Ready**，也就是说，Java中处于**Runnable**状态的线程有可能正在执行，也有可能没有正在执行，正在等待被分配CPU资源。
	所以，如果一个正在运行的线程是**Runnable**状态，当它运行到任务的一半时，执行该线程的CPU被调度去做其他事情，导致该线程暂时不运行，它的状态依然不变，还是**Runnable**，因为它有可能随时被调度回来继续执行任务。
-  **阻塞状态（Blocked）**
